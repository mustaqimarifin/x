...
const types = ["identifier", "keyword", "string", "class", "sign", "comment", "break", "space", "jsxliterals"];
 ...
 *
 */
const [T_IDENTIFIER, T_KEYWORD, T_STRING, T_CLS_NUMBER, T_SIGN, T_COMMENT, T_BREAK, T_SPACE, T_JSX_LITERALS] =
	types.map((_, i) => i);
...
function tokenize(code) {
	...
	function classify(token) {
		...
		return isIdentifier(token) && !inStrTemplateLiterals() && !isSingleQuotes(__strQuote) ? T_IDENTIFIER : T_STRING;
	}
	...
	for (let i = 0; i < code.length; i++) {
		...
		if (isQuotationChar || isStringTemplateLiterals || isSingleQuotes(__strQuote)) {
			...
			if (isRegexChar && lastType && !((lastType === T_SIGN && ")" !== lastToken) || lastType === T_COMMENT)) {
				current = curr;
				append();
				continue;
			}
...
export { highlight, tokenize, SugarHigh };
